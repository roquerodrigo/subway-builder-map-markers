...
function mountPanel(rect: { height: number, left: number, top: number, width: number }, position = 'fixed') {
  ...
  wrapper.getBoundingClientRect = () => ({ ...rect, right: rect.left + rect.width, bottom: rect.top + rect.height, x: rect.left, y: rect.top, toJSON: () => ({}) })
...
describe('clampStoredPanelGeometry', () => {
  ...
  it('leaves a geometry that is already on-screen untouched', () => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ x: 100, y: 80, width: 380, height: 560 }))
    const setItem = vi.spyOn(Storage.prototype, 'setItem')
    clampStoredPanelGeometry()
    expect(setItem).not.toHaveBeenCalled()
    setItem.mockRestore()
  ...
  it('pulls a stale off-screen position back inside the viewport before the game reads it', () => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ x: 3000, y: 80, width: 380, height: 560 }))
    clampStoredPanelGeometry()
    expect(storedGeometry()).toEqual({ x: 1412, y: 80, width: 380, height: 560 })
  })

  it('pulls a position back from above and to the left of the viewport', () => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ x: -500, y: -200, width: 380, height: 560 }))
    clampStoredPanelGeometry()
    expect(storedGeometry()).toEqual({ x: 8, y: 8, width: 380, height: 560 })
  })

  it('clamps against the bottom edge of the viewport', () => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ x: 100, y: 4000, width: 380, height: 560 }))
    clampStoredPanelGeometry()
  ...
  it('keeps the panel reachable when it is wider than the viewport', () => {
    setViewport(320, 240)
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ x: 3000, y: 4000, width: 380, height: 560 }))
    clampStoredPanelGeometry()
    expect(storedGeometry()).toEqual({ x: 8, y: 8, width: 380, height: 560 })
  })
...
describe('ensurePanelOnScreen', () => {
  ...
  it('does nothing when the panel has no fixed wrapper to move', () => {
    const { content } = mountPanel({ left: 3000, top: 40, width: 380, height: 560 }, 'static')
    ensurePanelOnScreen(content)
    expect(storedGeometry()).toBeNull()
  })

  it('leaves a window that is already fully on-screen alone', () => {
    const { content, wrapper } = mountPanel({ left: 100, top: 80, width: 380, height: 560 })
    ensurePanelOnScreen(content)
  ...
  it('pulls a window restored off the right edge back into reach', () => {
    const { content, wrapper } = mountPanel({ left: 3000, top: 40, width: 380, height: 560 })
    ensurePanelOnScreen(content)
  ...
  it('persists the clamped geometry so the fix survives the next open', () => {
    const { content } = mountPanel({ left: 3000, top: 40, width: 380, height: 560 })
    ensurePanelOnScreen(content)
    expect(storedGeometry()).toEqual({ x: 1412, y: 40, width: 380, height: 560 })
  })

  it('pulls a window restored above the viewport back into reach', () => {
    const { content, wrapper } = mountPanel({ left: 100, top: -400, width: 380, height: 560 })
    ensurePanelOnScreen(content)
  ...
  it('tolerates a sub-pixel offset rather than nudging the window', () => {
    const { content, wrapper } = mountPanel({ left: 8.4, top: 8.4, width: 380, height: 560 })
    ensurePanelOnScreen(content)
    expect(wrapper.style.left).toBe('')
    expect(storedGeometry()).toBeNull()
  })
